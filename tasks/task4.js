...
function getMostFuelEfficientCars(cars, brand) {
  return cars.every(car => car.fuelConsumption < 6) 
    ? cars.filter(car => car.brand === brand).sort((a, b) => a.fuelConsumption - b.fuelConsumption)
    : [];
}
...
const cars = [
  ...
  { brand: 'Ford', model: 'Focus', fuelConsumption: 4.1 },
